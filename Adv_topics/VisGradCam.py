...
class GradCAM:
    ...
    def compute_heatmap(self, image, classIdx, upsample_size, showRawHeatmap=False):
        ...
        if showRawHeatmap:
            ...
            _, axs = plt.subplots(nrows=1, ncols=2, squeeze=False, figsize=(6, 6))
            axs[0, 0].set_title('Raw heatmap')
            axs[0, 0].imshow(heatmap)
            axs[0, 0].axis('on')

            axs[0, 1].set_title('Upsampled heatmap')
            axs[0, 1].imshow(scaled_heatmap)
            axs[0, 1].axis('off')
            plt.tight_layout()
            plt.show()
...
#
#
# Guided Backpropagation is the combination of vanilla backpropagation at ReLUs and DeconvNets. 
...
#  Reference: https://www.naukri.com/code360/library/guided-backpropagation
@tf.custom_gradient
def guidedRelu(a):
  def grad(dy):
...
class GuidedBackprop:
    ...
    def __init__(self,model, layerName=None):
        ...
        layer_dict = [layer for layer in self.gbModel.layers[1:] if hasattr(layer,"activation")]
...
##########################################
# This class creates a new model with a modified output layer to allow visualisation of Feature Maps
# from the specified convulational layer.
# Useful utility for exploring how CNN work.
#
# Class is initialized using the model and the convulational layer you wish to see visualisation
##########################################
class CnnExplorer:
    ...
    def display_featuremap(self, image, square=8):
        ...
        # This will return an array of FeatureMaps (based on the dimensions of the conv layer)
        FeatureMaps = self.FeatureMapModel.predict(image)
        
        # Display the feature maps - the number of maps displays will be square^2
        ix = 0
        _, subs = plt.subplots(nrows=square, ncols=square, squeeze=False, figsize=(10, 10))
        for i in range(square):
            for j in range(square):
                subs[i,j].axis('off')
                # plot feature map  in grayscale
                subs[i,j].imshow(FeatureMaps[0, :, :, ix], cmap='gray')
                ix += 1
    ...
    def display_filters(self, numFilters=6):
        ...
        _, subs = plt.subplots(nrows=n_filters, ncols=3, squeeze=False, figsize=(10, 10))

        for i in range(n_filters):
            # get the filter
            f = filters[:, :, :, i]
            # plot each channel separately  (this is assuming there the layer has 3 channels)
            for j in range(3):
...
# Convert original based on requirement of CNN architecture
...
def overlayGradCAM(img, heatmap):
    # Normalization image to 0-255
    heatmap = np.uint8(255 * heatmap)
    # Use jet colormap to colorize heatmap
    heatmap = cv2.applyColorMap(heatmap, cv2.COLORMAP_JET)
    # Overlay image...adjusting image intensity ratio (0.3 for heatmap) (0.5 for original image)
...
def  get_params():
    ...
    parser.add_argument('--Image', default='./test_images/goldfish.jpeg', help='File Name of image to be explained')
    parser.add_argument('--ClassId', type=int, default=1, help='Image Classification ID')  
    parser.add_argument('--ConvLayer', default='conv5_block3_3_conv', help='Name of last Conv layer')
    parser.add_argument('--Model', default='ResNet50', help='ResNet50,  VGG16')  
    parser.add_argument('--FeatureMap', type=bool, default=False, help='Output Feature Map of ConvLayer')
    parser.add_argument('--Filters', type=int, default=False, help='Display the filters')  
    parser.add_argument('--Predict', type=bool, default=True, help='Run a predication against the image')  
    parser.add_argument('--ShowCams', type=bool, default=True, help='Show CAM generated images')  
    parser.add_argument('--Summary', type=bool, default=False, help='Show Model Summary') 
    parser.add_argument('--ShowRawHeatmap', type=bool, default=False, help='Show the raw CAM image before scaling') 
...
def showCAMs(originalImg, processImg, GradCAM, GuidedBP, ClassIdx, upsampleSize, showRawHeatmap=False):

    # Compute Gradcam heatmap and upscale to original image size 
    cam = GradCAM.compute_heatmap(image=processImg, classIdx=ClassIdx, upsample_size=upsampleSize, showRawHeatmap=showRawHeatmap)
    ...
    _, axs = plt.subplots(nrows=2, ncols=2, squeeze=False, figsize=(10, 10))


    axs[0, 0].set_title('Original image')
    axs[0, 0].imshow(cv2.cvtColor(originalImg, cv2.COLOR_BGR2RGB))
    axs[0, 0].axis('off')

    axs[0, 1].set_title('GradCam Heatmap')
    axs[0, 1].imshow(gradcam)
    axs[0, 1].axis('off')

    axs[1, 0].set_title('Guided Backpropagation')
    axs[1, 0].imshow(gb_im)
    axs[1, 0].axis('off')

    axs[1, 1].set_title('Masked Guided BP')
    axs[1, 1].imshow(guided_gradcam)
    axs[1, 1].axis('off')
...
if __name__ == "__main__":
    ...
    if args.Model == 'ResNet50':
        # Retrieve trained model (ResNet50), trained with ImageNet dataset (1000 classes)
    ...
    # If selected, display the trained filters
    # Specified Conv Layer
    if args.Filters > 0:
    ...
    if args.ShowCams:
        ...
        showCAMs(originalImg, processedImg, gradCAM, guidedBP, args.ClassId, upsampleSize, args.ShowRawHeatmap)